...
from .customs_browser import submit_to_customs_site
from .llm_extract import run_llm_extract
from .mineru_extractor import run_mineru_and_read_text, normalize_newlines, guess_text_from_outputs
from .spatial_extract import parse_region_rules, extract_fields_by_regions
from .text_extract import extract_kv_fields
...
def _build_extract_payload(
    *,
    ...
    allowed = [".pdf", ".png", ".jpg", ".jpeg", ".tiff", ".bmp", ".gif", ".doc", ".docx", ".ppt", ".pptx"]
    suffix = Path(file_name).suffix.lower()
    if suffix not in allowed:
        raise ValueError("仅支持 PDF/Office/图片文件")
    if not file_bytes:
    ...
    def run_single_extraction(current_bytes: bytes, rotation_angle: int) -> tuple[dict[str, Any], dict[str, Any], int, int]:
        ...
        text = normalize_newlines(outputs_local.get("text", ""))
        ...
        payload_local: dict[str, Any] = {
            ...
            "mineru_api": "official",
            "mineru_url": "https://mineru.net/api/v4/file-urls/batch",
            "model_version": actual_model_version,
            "backend": actual_model_version,
    ...
    for idx, (angle, candidate_bytes) in enumerate(candidates):
        stage = "running_mineru" if idx == 0 else "running_rotate_retry"
        msg = "解析中" if idx == 0 else f"低命中重试旋转 {angle}°"
